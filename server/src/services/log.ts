import Log, { ILog, ILogModel, LogServiceType, LogVerbosity } from "@/models/log";
import { IRangeResult } from "@/types/dto";
...
export default class LogService {
  ...
  async range(position : number, interval: number) : Promise<IRangeResult> {
    return {
      values: await this.logModel.find().skip((position - 1) * interval).limit(interval),
